  ...
  return {
    async Render(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Render("${params.p}")`, height);
      return parseGnoReturns(result) as RenderReturn;
    },
    async MustCreateProposal(
      params: { r: unknown },
      height?: number
    ): Promise<MustCreateProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `MustCreateProposal(${params.r})`, height);
      return parseGnoReturns(result) as MustCreateProposalReturn;
    },
    async ExecuteProposal(
      params: { pid: bigint },
      height?: number
    ): Promise<ExecuteProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `ExecuteProposal(${params.pid})`, height);
      return parseGnoReturns(result) as ExecuteProposalReturn;
    },
    async CreateProposal(
      params: { r: unknown },
      height?: number
    ): Promise<CreateProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `CreateProposal(${params.r})`, height);
      return parseGnoReturns(result) as CreateProposalReturn;
    },
    async MustVoteOnProposal(
      params: { r: unknown },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `MustVoteOnProposal(${params.r})`, height);
    },
    async VoteOnProposal(
      params: { r: unknown },
      height?: number
    ): Promise<VoteOnProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `VoteOnProposal(${params.r})`, height);
      return parseGnoReturns(result) as VoteOnProposalReturn;
    },
    async MustVoteOnProposalSimple(
      params: { pid: bigint; option: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `MustVoteOnProposalSimple(${params.pid},"${params.option}")`,
          height
        );
    },
    async MustGetProposal(
      params: { pid: bigint },
      height?: number
    ): Promise<MustGetProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `MustGetProposal(${params.pid})`, height);
      return parseGnoReturns(result) as MustGetProposalReturn;
    },
    async GetProposal(
      params: { pid: bigint },
      height?: number
    ): Promise<GetProposalReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetProposal(${params.pid})`, height);
      return parseGnoReturns(result) as GetProposalReturn;
    },
    async UpdateImpl(params: { r: unknown }, height?: number): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `UpdateImpl(${params.r})`, height);
    },
    async AllowedDAOs(height?: number): Promise<AllowedDAOsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `AllowedDAOs()`, height);
      return parseGnoReturns(result) as AllowedDAOsReturn;
    },
    async InAllowedDAOs(
      params: { pkg: string },
      height?: number
    ): Promise<InAllowedDAOsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `InAllowedDAOs("${params.pkg}")`, height);
      return parseGnoReturns(result) as InAllowedDAOsReturn;
    },
    async NewProposalRequest(
      params: { title: string; description: string },
      height?: number
    ): Promise<NewProposalRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewProposalRequest("${params.title}","${params.description}")`,
          height
        );
      return parseGnoReturns(result) as NewProposalRequestReturn;
    },
    async NewProposalRequestWithFilter(
      params: { title: string; description: string },
      height?: number
    ): Promise<NewProposalRequestWithFilterReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewProposalRequestWithFilter("${params.title}","${params.description}")`,
          height
        );
    ...
    async NewProposals(height?: number): Promise<NewProposalsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `NewProposals()`, height);
      return parseGnoReturns(result) as NewProposalsReturn;
    },
    async NewSimpleExecutor(
      params: { callback: unknown; description: string },
      height?: number
    ): Promise<NewSimpleExecutorReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewSimpleExecutor(${params.callback},"${params.description}")`,
          height
        );
    ...
    async NewSafeExecutor(height?: number): Promise<NewSafeExecutorReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `NewSafeExecutor()`, height);
...
const txClient = (wallet: GnoWallet) => {
  return {
    ...
    async NewSafeExecutor(
      funds: Map<string, number>,
      maxDeposit: Map<string, number>,
      fee: TxFee
    ): Promise<NewSafeExecutorReturn> {
      const resp = await wallet.callMethod(
        realm,
        'NewSafeExecutor',
        [],
