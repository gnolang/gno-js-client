...
const queryClient = (wallet: GnoWallet) => {
	return {
		async Init(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Init()`, height);
		},
		async InitWithUsers(params: { addrs: unknown }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `InitWithUsers(${params.addrs})`, height);
		}
	}
}
...
const txClient = (wallet: GnoWallet) => {
	return {
		...
		async InitWithUsers(params: { addrs: unknown }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"InitWithUsers",
				[String(params.addrs)],
...
const Realm = (wallet: GnoWallet) => {
	return {
		realm: { realms: { r: { gov: { dao: { v3: { init: new RealmModule(wallet) } } } } } }
	}
}
