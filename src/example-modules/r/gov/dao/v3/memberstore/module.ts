...
const queryClient = (wallet: GnoWallet) => {
  return {
    ...
    async Get(height?: number): Promise<GetReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Get()`, height);
      return parseGnoReturns(result) as GetReturn;
    },
    async GetTier(
      params: { name: string },
      height?: number
    ): Promise<GetTierReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetTier("${params.name}")`, height);
    ...
    async IterateTiers(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `IterateTiers(${params.fn})`, height);
    },
    async GetTierPower(
      params: { tierName: string; members: unknown },
      height?: number
    ): Promise<GetTierPowerReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `GetTierPower("${params.tierName}",${params.members})`,
          height
        );
      return parseGnoReturns(result) as GetTierPowerReturn;
    },
    async NewChangeTiersRequest(
      params: { tiers: unknown },
      height?: number
    ): Promise<NewChangeTiersRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewChangeTiersRequest(${params.tiers})`,
          height
        );
      return parseGnoReturns(result) as NewChangeTiersRequestReturn;
    },
    async RenderCharts(
      params: { members: unknown },
      height?: number
    ): Promise<RenderChartsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `RenderCharts(${params.members})`, height);
      return parseGnoReturns(result) as RenderChartsReturn;
    },
    async RenderMembers(
      params: { path: string; members: unknown },
      height?: number
    ): Promise<RenderMembersReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `RenderMembers("${params.path}",${params.members})`,
          height
        );
    ...
    async NewMembersByTier(height?: number): Promise<NewMembersByTierReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `NewMembersByTier()`, height);
...
const txClient = (wallet: GnoWallet) => {
  return {
    ...
    async NewMembersByTier(
      funds: Map<string, number>,
      maxDeposit: Map<string, number>,
      fee: TxFee
    ): Promise<NewMembersByTierReturn> {
      const resp = await wallet.callMethod(
        realm,
        'NewMembersByTier',
        [],
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: {
        r: { gov: { dao: { v3: { memberstore: new RealmModule(wallet) } } } },
      },
    },
  };
};
