// Auto-generated module for gno.land/r/gov/dao/v3/treasury — DO NOT EDIT
import {
  GnoWallet,
...
const queryClient = (wallet: GnoWallet) => {
  return {
    async SetTokenKeys(params: {
      keys: unknown
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `SetTokenKeys(${params.keys})`, height);
      return;
    },
    async Send(height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, "Send()", height);
      return;
    },
    async History(params: {
      ...
      pageNumber: bigint
      pageSize: bigint
    }, height?: number): Promise<HistoryReturn> {
    ...
    async ListBankerIDs(height?: number): Promise<ListBankerIDsReturn> {
      ...
      return _parseGnoReturns(result) as ListBankerIDsReturn;
    },
    async Render(params: {
      path: string
...
const txClient = (wallet: GnoWallet) => {
  return {
    ...
    async History(params: {
      ...
      pageNumber: bigint
      pageSize: bigint
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<HistoryReturn> {
      const resp = await wallet.callMethod(
        realm,
        "History",
    ...
    async Balances(params: {
      bankerID: string
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<BalancesReturn> {
      const resp = await wallet.callMethod(
        realm,
        "Balances",
    ...
    async Address(params: {
      bankerID: string
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<AddressReturn> {
      const resp = await wallet.callMethod(
        realm,
        "Address",
        [String(params.bankerID)],
    ...
    async HasBanker(params: {
      bankerID: string
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<HasBankerReturn> {
      const resp = await wallet.callMethod(
        realm,
        "HasBanker",
        [String(params.bankerID)],
      ...
      realms: {
        r: {
          gov: {
            dao: {
              v3: {
                treasury: new RealmModule(wallet),
              },
            },
          },
        },
