  ...
  return {
    async SetTokenKeys(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `SetTokenKeys(${params.keys})`, height);
    },
    async Send(height?: number): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Send()`, height);
    },
    async History(
      params: { bankerID: string; pageNumber: bigint; pageSize: bigint },
      height?: number
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `History("${params.bankerID}",${params.pageNumber},${params.pageSize})`,
          height
        );
      return parseGnoReturns(result) as HistoryReturn;
    },
    async Balances(
      params: { bankerID: string },
      height?: number
    ): Promise<BalancesReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Balances("${params.bankerID}")`, height);
    ...
    async Address(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Address("${params.bankerID}")`, height);
      return parseGnoReturns(result) as AddressReturn;
    },
    async HasBanker(
      params: { bankerID: string },
      height?: number
    ): Promise<HasBankerReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `HasBanker("${params.bankerID}")`, height);
      return parseGnoReturns(result) as HasBankerReturn;
    },
    async ListBankerIDs(height?: number): Promise<ListBankerIDsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `ListBankerIDs()`, height);
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: {
        r: { gov: { dao: { v3: { treasury: new RealmModule(wallet) } } } },
      },
    },
  };
};
