  ...
  return {
    async New(
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `New("${params.name}","${params.symbol}",${params.decimals},${params.initialMint},${params.faucet})`,
          height
        );
    },
    async NewWithAdmin(
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `NewWithAdmin("${params.name}","${params.symbol}",${params.decimals},${params.initialMint},${params.faucet},"${params.admin}")`,
          height
        );
    },
    async Bank(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Bank("${params.symbol}")`, height);
    ...
    async TotalSupply(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `TotalSupply("${params.symbol}")`, height);
      return parseGnoReturns(result) as TotalSupplyReturn;
    },
    async HasAddr(
      params: { symbol: string; owner: string },
      height?: number
    ): Promise<HasAddrReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `HasAddr("${params.symbol}","${params.owner}")`,
          height
        );
    ...
    async BalanceOf(
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `BalanceOf("${params.symbol}","${params.owner}")`,
          height
        );
      return parseGnoReturns(result) as BalanceOfReturn;
    },
    async Allowance(
      params: { symbol: string; owner: string; spender: string },
      height?: number
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `Allowance("${params.symbol}","${params.owner}","${params.spender}")`,
          height
        );
      return parseGnoReturns(result) as AllowanceReturn;
    },
    async Transfer(
      params: { symbol: string; to: string; amount: bigint },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `Transfer("${params.symbol}","${params.to}",${params.amount})`,
          height
        );
    },
    async Approve(
      params: { symbol: string; spender: string; amount: bigint },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `Approve("${params.symbol}","${params.spender}",${params.amount})`,
          height
        );
    },
    async TransferFrom(
      params: { symbol: string; from: string; to: string; amount: bigint },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `TransferFrom("${params.symbol}","${params.from}","${params.to}",${params.amount})`,
          height
        );
    },
    async Faucet(params: { symbol: string }, height?: number): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Faucet("${params.symbol}")`, height);
    },
    async Mint(
      params: { symbol: string; to: string; amount: bigint },
      height?: number
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `Mint("${params.symbol}","${params.to}",${params.amount})`,
          height
        );
    },
    async Burn(
      params: { symbol: string; from: string; amount: bigint },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `Burn("${params.symbol}","${params.from}",${params.amount})`,
          height
        );
    },
    async DropInstanceOwnership(
      params: { symbol: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `DropInstanceOwnership("${params.symbol}")`,
          height
        );
    },
    async TransferInstanceOwnership(
      params: { symbol: string; newOwner: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `TransferInstanceOwnership("${params.symbol}","${params.newOwner}")`,
          height
        );
    },
    async ListTokens(
      params: { pageNumber: bigint; pageSize: bigint },
      height?: number
    ): Promise<ListTokensReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `ListTokens(${params.pageNumber},${params.pageSize})`,
          height
        );
    ...
    async New(
      ...
      const resp = await wallet.callMethod(
        ...
        [
          ...
          String(params.initialMint),
          String(params.faucet),
        ],
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: {
        r: { demo: { defi: { grc20factory: new RealmModule(wallet) } } },
      },
    },
  };
};
