...
const queryClient = (wallet: GnoWallet) => {
	return {
		async Register(params: { token: unknown; slug: string }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Register(${params.token},"${params.slug}")`, height);
		},
		async Get(params: { key: string }, height?: number):Promise<GetReturn> {
		...
		async MustGet(params: { key: string }, height?: number):Promise<MustGetReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `MustGet("${params.key}")`, height);
			return parseGnoReturns(result) as MustGetReturn;
		},
		...
		async GetRegistry(height?: number):Promise<GetRegistryReturn> {
			...
			return parseGnoReturns(result) as GetRegistryReturn;
		}
	}
}
...
const txClient = (wallet: GnoWallet) => {
	return {
		async Register(params: { token: unknown; slug: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
		...
		async Get(params: { key: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<GetReturn> {
			const resp = await wallet.callMethod(
				realm,
				"Get",
				[String(params.key)],
		...
		async MustGet(params: { key: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<MustGetReturn> {
			const resp = await wallet.callMethod(
				realm,
				"MustGet",
				[String(params.key)],
...
const Realm = (wallet: GnoWallet) => {
	return {
		realm: { realms: { r: { demo: { defi: { grc20reg: new RealmModule(wallet) } } } } }
	}
}
