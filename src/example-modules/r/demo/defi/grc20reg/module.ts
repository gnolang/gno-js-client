...
const queryClient = (wallet: GnoWallet) => {
  return {
    async Register(
      params: { token: unknown; slug: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `Register(${params.token},"${params.slug}")`,
          height
        );
    },
    async Get(params: { key: string }, height?: number): Promise<GetReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Get("${params.key}")`, height);
      return parseGnoReturns(result) as GetReturn;
    },
    async MustGet(
      params: { key: string },
      height?: number
    ): Promise<MustGetReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `MustGet("${params.key}")`, height);
    ...
    async GetRegistry(height?: number): Promise<GetRegistryReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetRegistry()`, height);
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: { r: { demo: { defi: { grc20reg: new RealmModule(wallet) } } } },
    },
  };
};
