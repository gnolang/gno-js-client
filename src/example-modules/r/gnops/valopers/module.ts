  ...
  return {
    async Auth(height?: number): Promise<AuthReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Auth()`, height);
      return parseGnoReturns(result) as AuthReturn;
    },
    async NewInstructionsProposalCallback(
      params: { newInstructions: string },
      height?: number
    ): Promise<NewInstructionsProposalCallbackReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewInstructionsProposalCallback("${params.newInstructions}")`,
          height
        );
      return parseGnoReturns(result) as NewInstructionsProposalCallbackReturn;
    },
    async NewMinFeeProposalCallback(
      params: { newMinFee: bigint },
      height?: number
    ): Promise<NewMinFeeProposalCallbackReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewMinFeeProposalCallback(${params.newMinFee})`,
          height
        );
      return parseGnoReturns(result) as NewMinFeeProposalCallbackReturn;
    },
    async AddToAuthList(
      params: { address_XXX: string; member: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `AddToAuthList("${params.address_XXX}","${params.member}")`,
          height
        );
    },
    async DeleteFromAuthList(
      params: { address_XXX: string; member: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `DeleteFromAuthList("${params.address_XXX}","${params.member}")`,
          height
        );
    },
    async Register(
      ...
      const result = await wallet
        ...
        .evaluateExpression(
          realm,
          `Register("${params.moniker}","${params.description}","${params.serverType}","${params.address_XXX}","${params.pubKey}")`,
          height
        );
    },
    async UpdateMoniker(
      params: { address_XXX: string; moniker: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `UpdateMoniker("${params.address_XXX}","${params.moniker}")`,
          height
        );
    },
    async UpdateDescription(
      params: { address_XXX: string; description: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `UpdateDescription("${params.address_XXX}","${params.description}")`,
          height
        );
    },
    async UpdateKeepRunning(
      params: { address_XXX: string; keepRunning: boolean },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `UpdateKeepRunning("${params.address_XXX}",${params.keepRunning})`,
          height
        );
    },
    async UpdateServerType(
      params: { address_XXX: string; serverType: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `UpdateServerType("${params.address_XXX}","${params.serverType}")`,
          height
        );
    },
    async GetByAddr(
      params: { address_XXX: string },
      height?: number
    ): Promise<GetByAddrReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `GetByAddr("${params.address_XXX}")`,
          height
        );
    ...
    async Render(
      ...
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Render("${params.fullPath}")`, height);
  ...
  return {
    ...
    async Register(
      ...
      const resp = await wallet.callMethod(
        ...
        'Register',
        [
          String(params.moniker),
          String(params.description),
          String(params.serverType),
