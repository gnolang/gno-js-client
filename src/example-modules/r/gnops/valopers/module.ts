// Auto-generated module for gno.land/r/gnops/valopers — DO NOT EDIT
import {
  GnoWallet,
...
const queryClient = (wallet: GnoWallet) => {
  return {
    async Auth(height?: number): Promise<AuthReturn> {
      ...
      return _parseGnoReturns(result) as AuthReturn;
    },
    async NewInstructionsProposalCallback(params: {
      newInstructions: string
    }, height?: number): Promise<NewInstructionsProposalCallbackReturn> {
      ...
      return _parseGnoReturns(result) as NewInstructionsProposalCallbackReturn;
    },
    async NewMinFeeProposalCallback(params: {
      newMinFee: bigint
    }, height?: number): Promise<NewMinFeeProposalCallbackReturn> {
      const result = await wallet.getProvider().evaluateExpression(realm, `NewMinFeeProposalCallback(${params.newMinFee})`, height);
      return _parseGnoReturns(result) as NewMinFeeProposalCallbackReturn;
    },
    async AddToAuthList(params: {
      address_XXX: string
      member: string
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `AddToAuthList("${params.address_XXX}","${params.member}")`, height);
      return;
    },
    async DeleteFromAuthList(params: {
      address_XXX: string
      member: string
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `DeleteFromAuthList("${params.address_XXX}","${params.member}")`, height);
      return;
    },
    async Register(params: {
      ...
      await wallet.getProvider().evaluateExpression(realm, `Register("${params.moniker}","${params.description}","${params.serverType}","${params.address_XXX}","${params.pubKey}")`, height);
      return;
    },
    async UpdateMoniker(params: {
      address_XXX: string
      moniker: string
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `UpdateMoniker("${params.address_XXX}","${params.moniker}")`, height);
      return;
    },
    async UpdateDescription(params: {
      ...
      await wallet.getProvider().evaluateExpression(realm, `UpdateDescription("${params.address_XXX}","${params.description}")`, height);
      return;
    },
    async UpdateKeepRunning(params: {
      address_XXX: string
      keepRunning: boolean
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `UpdateKeepRunning("${params.address_XXX}",${params.keepRunning})`, height);
      return;
    },
    async UpdateServerType(params: {
      address_XXX: string
      serverType: string
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `UpdateServerType("${params.address_XXX}","${params.serverType}")`, height);
      return;
    },
    async GetByAddr(params: {
      address_XXX: string
    }, height?: number): Promise<GetByAddrReturn> {
      const result = await wallet.getProvider().evaluateExpression(realm, `GetByAddr("${params.address_XXX}")`, height);
      return _parseGnoReturns(result) as GetByAddrReturn;
    },
    async Render(params: {
      fullPath: string
...
const txClient = (wallet: GnoWallet) => {
  return {
    ...
    async NewInstructionsProposalCallback(params: {
      newInstructions: string
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<NewInstructionsProposalCallbackReturn> {
      const resp = await wallet.callMethod(
        realm,
    ...
    async NewMinFeeProposalCallback(params: {
      newMinFee: bigint
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<NewMinFeeProposalCallbackReturn> {
      const resp = await wallet.callMethod(
        realm,
    ...
    async GetByAddr(params: {
      address_XXX: string
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<GetByAddrReturn> {
      const resp = await wallet.callMethod(
        realm,
        "GetByAddr",
  ...
  return {
    realm: {
      realms: {
        r: {
          gnops: {
            valopers: new RealmModule(wallet),
          },
        },
      },
    },
