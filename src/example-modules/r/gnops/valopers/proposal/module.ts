...
const queryClient = (wallet: GnoWallet) => {
  return {
    async NewValidatorProposalRequest(
      params: { address_XXX: string },
      height?: number
    ): Promise<NewValidatorProposalRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewValidatorProposalRequest("${params.address_XXX}")`,
          height
        );
      return parseGnoReturns(result) as NewValidatorProposalRequestReturn;
    },
    async ProposeNewInstructionsProposalRequest(
      params: { newInstructions: string },
      height?: number
    ): Promise<ProposeNewInstructionsProposalRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `ProposeNewInstructionsProposalRequest("${params.newInstructions}")`,
          height
        );
      return parseGnoReturns(
        result
      ) as ProposeNewInstructionsProposalRequestReturn;
    },
    async ProposeNewMinFeeProposalRequest(
      params: { newMinFee: bigint },
      height?: number
    ): Promise<ProposeNewMinFeeProposalRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `ProposeNewMinFeeProposalRequest(${params.newMinFee})`,
          height
        );
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: {
        r: { gnops: { valopers: { proposal: new RealmModule(wallet) } } },
      },
    },
  };
};
