...
const queryClient = (wallet: GnoWallet) => {
	return {
		async SetAdmin(params: { admin: string }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `SetAdmin("${params.admin}")`, height);
		},
		async SetOpenAccountAmount(params: { amount: bigint }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `SetOpenAccountAmount(${params.amount})`, height);
		},
...
const txClient = (wallet: GnoWallet) => {
	return {
		async SetAdmin(params: { admin: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
		...
		async SetOpenAccountAmount(params: { amount: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
		...
		async New(params: { owner: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<NewReturn> {
			const resp = await wallet.callMethod(
				realm,
				"New",
				[String(params.owner)],
...
const Realm = (wallet: GnoWallet) => {
	return {
		realm: { realms: { r: { g1hy6zry03hg5d8le9s2w4fxme6236hkgd928dun: { boards2: { permissions: { v1: new RealmModule(wallet) } } } } } }
	}
}
