...
const queryClient = (wallet: GnoWallet) => {
	return {
		async Increment(params: { change: bigint }, height?: number):Promise<IncrementReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Increment(${params.change})`, height);
...
const txClient = (wallet: GnoWallet) => {
	return {
		async Increment(params: { change: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<IncrementReturn> {
...
const Realm = (wallet: GnoWallet) => {
	return {
		realm: { realms: { r: { g17wparzzj7stjzyxthf32csewkgr54ganvsmdc9: { counter: new RealmModule(wallet) } } } }
	}
}
