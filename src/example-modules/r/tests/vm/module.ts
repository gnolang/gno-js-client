...
const queryClient = (wallet: GnoWallet) => {
	return {
		async AddStringer(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `AddStringer()`, height);
		},
		...
		async IncCounter(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `IncCounter()`, height);
		},
		...
		async CallSubtestsAssertOriginCall(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `CallSubtestsAssertOriginCall()`, height);
		},
		...
		async ModifyTestRealmObject(params: { t: unknown }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ModifyTestRealmObject(${params.t})`, height);
		},
		async InitTestNodes(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `InitTestNodes()`, height);
		},
		async ModTestNodes(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ModTestNodes()`, height);
		},
		async PrintTestNodes(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `PrintTestNodes()`, height);
		},
		...
		async ExecSwitch(params: { fn: unknown }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ExecSwitch(${params.fn})`, height);
		},
...
const txClient = (wallet: GnoWallet) => {
	return {
		...
		async ModifyTestRealmObject(params: { t: unknown }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
		...
		async ExecSwitch(params: { fn: unknown }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"ExecSwitch",
				[String(params.fn)],
