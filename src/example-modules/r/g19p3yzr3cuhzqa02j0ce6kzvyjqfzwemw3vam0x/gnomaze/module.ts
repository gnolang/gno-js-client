// Auto-generated module for gno.land/r/g19p3yzr3cuhzqa02j0ce6kzvyjqfzwemw3vam0x/gnomaze — DO NOT EDIT
import {
  GnoWallet,
...
const queryClient = (wallet: GnoWallet) => {
  return {
    async GenerateNextLevel(params: {
      answer: string
    }, height?: number): Promise<void> {
      await wallet.getProvider().evaluateExpression(realm, `GenerateNextLevel("${params.answer}")`, height);
      return;
    },
    async GenerateSVG(params: {
      p: unknown
    }, height?: number): Promise<GenerateSVGReturn> {
      const result = await wallet.getProvider().evaluateExpression(realm, `GenerateSVG(${params.p})`, height);
      return _parseGnoReturns(result) as GenerateSVGReturn;
    },
    async Render(params: {
      path: string
...
const txClient = (wallet: GnoWallet) => {
  return {
    ...
    async GenerateSVG(params: {
      p: unknown
    }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee): Promise<GenerateSVGReturn> {
      const resp = await wallet.callMethod(
        realm,
        "GenerateSVG",
        [String(params.p)],
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: {
        r: {
          g19p3yzr3cuhzqa02j0ce6kzvyjqfzwemw3vam0x: {
            gnomaze: new RealmModule(wallet),
          },
        },
      },
    },
