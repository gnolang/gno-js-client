...
const queryClient = (wallet: GnoWallet) => {
	return {
		async ProposeNewCLA(params: { newHash: string; newURL: string }, height?: number):Promise<ProposeNewCLAReturn> {
		...
		async Sign(params: { hash: string }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Sign("${params.hash}")`, height);
		},
		async HasValidSignature(params: { addr: string }, height?: number):Promise<HasValidSignatureReturn> {
...
const txClient = (wallet: GnoWallet) => {
	return {
		async ProposeNewCLA(params: { newHash: string; newURL: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ProposeNewCLAReturn> {
		...
		async Sign(params: { hash: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"Sign",
				[String(params.hash)],
		...
		async HasValidSignature(params: { addr: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<HasValidSignatureReturn> {
			const resp = await wallet.callMethod(
				realm,
				"HasValidSignature",
				[String(params.addr)],
