...
const queryClient = (wallet: GnoWallet) => {
	return {
		async AddControllerAtGenesis(params: { addr: string }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `AddControllerAtGenesis("${params.addr}")`, height);
		},
		async ProposeNewController(params: { addr: string }, height?: number):Promise<ProposeNewControllerReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ProposeNewController("${params.addr}")`, height);
			return parseGnoReturns(result) as ProposeNewControllerReturn;
		},
		async ProposeControllerRemoval(params: { addr: string }, height?: number):Promise<ProposeControllerRemovalReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ProposeControllerRemoval("${params.addr}")`, height);
			return parseGnoReturns(result) as ProposeControllerRemovalReturn;
		},
		async ProposeControllerAdditionAndRemoval(params: { toAdd: string; toRemove: string }, height?: number):Promise<ProposeControllerAdditionAndRemovalReturn> {
			...
			return parseGnoReturns(result) as ProposeControllerAdditionAndRemovalReturn;
		},
		async NewErrNotWhitelisted(height?: number):Promise<NewErrNotWhitelistedReturn> {
			...
			return parseGnoReturns(result) as NewErrNotWhitelistedReturn;
		},
		...
		async RegisterUser(params: { name: string; address_XXX: string }, height?: number):Promise<RegisterUserReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `RegisterUser("${params.name}","${params.address_XXX}")`, height);
			return parseGnoReturns(result) as RegisterUserReturn;
		},
		async ResolveName(params: { name: string }, height?: number):Promise<ResolveNameReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ResolveName("${params.name}")`, height);
			return parseGnoReturns(result) as ResolveNameReturn;
		},
		async ResolveAddress(params: { addr: string }, height?: number):Promise<ResolveAddressReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ResolveAddress("${params.addr}")`, height);
			return parseGnoReturns(result) as ResolveAddressReturn;
		},
		async ResolveAny(params: { input: string }, height?: number):Promise<ResolveAnyReturn> {
			const result = await wallet.getProvider().evaluateExpression(realm, `ResolveAny("${params.input}")`, height);
			return parseGnoReturns(result) as ResolveAnyReturn;
		},
		async GetReadonlyAddrStore(height?: number):Promise<GetReadonlyAddrStoreReturn> {
			...
			return parseGnoReturns(result) as GetReadonlyAddrStoreReturn;
		},
		async GetReadOnlyNameStore(height?: number):Promise<GetReadOnlyNameStoreReturn> {
			...
			return parseGnoReturns(result) as GetReadOnlyNameStoreReturn;
		}
	}
}
...
const txClient = (wallet: GnoWallet) => {
	return {
		async AddControllerAtGenesis(params: { addr: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
		...
		async ProposeNewController(params: { addr: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ProposeNewControllerReturn> {
			const resp = await wallet.callMethod(
				realm,
		...
		async ProposeControllerRemoval(params: { addr: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ProposeControllerRemovalReturn> {
			const resp = await wallet.callMethod(
				realm,
		...
		async ProposeControllerAdditionAndRemoval(params: { toAdd: string; toRemove: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ProposeControllerAdditionAndRemovalReturn> {
			const resp = await wallet.callMethod(
				realm,
		...
		async RegisterUser(params: { name: string; address_XXX: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<RegisterUserReturn> {
			const resp = await wallet.callMethod(
				realm,
				"RegisterUser",
		...
		async ResolveName(params: { name: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ResolveNameReturn> {
			const resp = await wallet.callMethod(
				realm,
				"ResolveName",
				[String(params.name)],
		...
		async ResolveAddress(params: { addr: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ResolveAddressReturn> {
			const resp = await wallet.callMethod(
				realm,
				"ResolveAddress",
				[String(params.addr)],
		...
		async ResolveAny(params: { input: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<ResolveAnyReturn> {
			const resp = await wallet.callMethod(
				realm,
				"ResolveAny",
				[String(params.input)],
...
const Realm = (wallet: GnoWallet) => {
	return {
		realm: { realms: { r: { sys: { users: new RealmModule(wallet) } } } }
	}
}
