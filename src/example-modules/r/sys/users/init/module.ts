  ...
  return {
    async Bootstrap(height?: number): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `Bootstrap()`, height);
    },
    async RegisterUser(
      params: { name: string; addr: string },
      height?: number
    ): Promise<void> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `RegisterUser("${params.name}","${params.addr}")`,
          height
        );
    },
  };
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: { r: { sys: { users: { init: new RealmModule(wallet) } } } },
    },
  };
};
