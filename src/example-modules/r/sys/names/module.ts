...
const queryClient = (wallet: GnoWallet) => {
	return {
		...
		async IsAuthorizedAddressForNamespace(params: { address_XXX: string; namespace: string }, height?: number):Promise<IsAuthorizedAddressForNamespaceReturn> {
		...
		async Enable(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Enable()`, height);
		},
...
const txClient = (wallet: GnoWallet) => {
	return {
		...
		async IsAuthorizedAddressForNamespace(params: { address_XXX: string; namespace: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<IsAuthorizedAddressForNamespaceReturn> {
			const resp = await wallet.callMethod(
				realm,
