...
const queryClient = (wallet: GnoWallet) => {
  return {
    async GetChanges(
      params: { from: bigint },
      height?: number
    ): Promise<GetChangesReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetChanges(${params.from})`, height);
      return parseGnoReturns(result) as GetChangesReturn;
    },
    async NewPropRequest(
      params: { changesFn: unknown; title: string; description: string },
      height?: number
    ): Promise<NewPropRequestReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(
          realm,
          `NewPropRequest(${params.changesFn},"${params.title}","${params.description}")`,
          height
        );
    ...
    async IsValidator(
      params: { addr: string },
      height?: number
    ): Promise<IsValidatorReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `IsValidator("${params.addr}")`, height);
      return parseGnoReturns(result) as IsValidatorReturn;
    },
    async GetValidator(
      params: { addr: string },
      height?: number
    ): Promise<GetValidatorReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetValidator("${params.addr}")`, height);
      return parseGnoReturns(result) as GetValidatorReturn;
    },
    async GetValidators(height?: number): Promise<GetValidatorsReturn> {
      const result = await wallet
        .getProvider()
        .evaluateExpression(realm, `GetValidators()`, height);
...
const Realm = (wallet: GnoWallet) => {
  return {
    realm: {
      realms: { r: { sys: { validators: { v2: new RealmModule(wallet) } } } },
    },
  };
};
