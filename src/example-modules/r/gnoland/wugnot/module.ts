...
const queryClient = (wallet: GnoWallet) => {
	return {
		async Deposit(height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Deposit()`, height);
		},
		async Withdraw(params: { amount: bigint }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Withdraw(${params.amount})`, height);
		},
		...
		async BalanceOf(params: { owner: string }, height?: number):Promise<BalanceOfReturn> {
		...
		async Allowance(params: { owner: string; spender: string }, height?: number):Promise<AllowanceReturn> {
		...
		async Transfer(params: { to: string; amount: bigint }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Transfer("${params.to}",${params.amount})`, height);
		},
		async Approve(params: { spender: string; amount: bigint }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `Approve("${params.spender}",${params.amount})`, height);
		},
		async TransferFrom(params: { from: string; to: string; amount: bigint }, height?: number):Promise<void> {
			const result = await wallet.getProvider().evaluateExpression(realm, `TransferFrom("${params.from}","${params.to}",${params.amount})`, height);
		}
	}
}
...
const txClient = (wallet: GnoWallet) => {
	return {
		...
		async Withdraw(params: { amount: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"Withdraw",
		...
		async BalanceOf(params: { owner: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<BalanceOfReturn> {
			const resp = await wallet.callMethod(
				realm,
				"BalanceOf",
				[String(params.owner)],
		...
		async Allowance(params: { owner: string; spender: string }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<AllowanceReturn> {
			const resp = await wallet.callMethod(
				realm,
				"Allowance",
		...
		async Transfer(params: { to: string; amount: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"Transfer",
		...
		async Approve(params: { spender: string; amount: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"Approve",
		...
		async TransferFrom(params: { from: string; to: string; amount: bigint }, funds: Map<string, number>, maxDeposit: Map<string, number>, fee: TxFee):Promise<void> {
			const resp = await wallet.callMethod(
				realm,
				"TransferFrom",
